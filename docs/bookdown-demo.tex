...
The root that satsifies the above equation is: \[ L^*=\frac{1}{\phi_1}\]
...
\[ y_t=\phi_0 +\phi_1 y_{t-1}+ \epsilon_t \quad ; \ \epsilon_t\sim WN(0, \sigma^2) \ and \  |\phi_1|<1\]
...
When estimating the AR model we have two alternatives: 1. OLS: biased
(but consistent) estimates. Also, later on when we add MA components we
cannot use OLS.
...
likelihood function, which assuming indpendently distributed random
sample, gives us:
...
multiplying individual probablity density functions to obtain the joint
...
need to estimate becomes even more compplicated.
...
\[f_{t,1}=E(y_{t+1}|\Omega_t)=\phi_0+\phi_1 y_t\] Similaryl, the
