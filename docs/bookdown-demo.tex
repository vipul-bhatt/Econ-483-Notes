...
To implement this test, we estimate equation \eqref{eq:bg} by replacing
the error term with the OLS residuals. The LM test statistic is given
by:
...
about the unknown future value of a time series of interest. For
example, what is the stock price of Facebook next Monday?
...
\begin{equation}
\frac{\partial y_{t+1}}{\partial x_t}  =0; \ \frac{\partial y_{t+1}}{\partial x_t}=\beta_1; \ \frac{\partial y_{t+2}}{\partial x_t}=\beta_2; \ \frac{\partial y_{t+s}}{\partial x_t}=0 \ \forall \ s>2 
\end{equation}
...
and capital stock. Our model specification is given by:
...
Figure \ref{fig:ch6-figure4} below shows the PACF for a stationary time
series where only one lag directly affects the time series in the
current period.
